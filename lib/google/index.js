...
function parseResult(resultString, payment) {
	...
	result.packageName = payment.packageName;
	result.productId = payment.productId;
	result.purchaseToken =  payment.receipt;
...
exports.isCancelled = function (response, cb) {
	try {
		let receipt = response.receipt;
		let res = (receipt.cancelReason && receipt.cancelReason !== CANCELLATION_REASON.USER_CANCELLED);
...
exports.isExpired = function (response, cb) {
	try {
		let receipt = response.receipt;
		let res = false;

		if (!receipt.expiryTimeMillis) {
			res = false;
		}

		if (Date.now() - receipt.expiryTimeMillis >= 0) {
			res = true;
		}
...
exports.acknowledge = function (payment, cb) {
	let keyObject;
